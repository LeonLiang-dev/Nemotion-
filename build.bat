...
if not exist "%PROJECT_DIR%sql\migrations\V3_expand_auth_password.sql" (
    echo ERROR: Missing migration SQL: "%PROJECT_DIR%sql\migrations\V3_expand_auth_password.sql"
    exit /b 1
)

if not exist "%PROJECT_DIR%sql\migrations\V4_fix_card_overtime_default.sql" (
    echo ERROR: Missing migration SQL: "%PROJECT_DIR%sql\migrations\V4_fix_card_overtime_default.sql"
    exit /b 1
)
...
if not exist "%PROJECT_DIR%sql\migrations\V7_fix_room_audit_defaults.sql" (
    echo ERROR: Missing migration SQL: "%PROJECT_DIR%sql\migrations\V7_fix_room_audit_defaults.sql"
    exit /b 1
)

if not exist "%PROJECT_DIR%sql\migrations\V8_fix_exam_create_defaults.sql" (
    echo ERROR: Missing migration SQL: "%PROJECT_DIR%sql\migrations\V8_fix_exam_create_defaults.sql"
    exit /b 1
)
